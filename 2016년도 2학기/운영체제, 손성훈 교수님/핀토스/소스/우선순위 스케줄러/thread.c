...
static bool exp_less (const struct list_elem *a,const struct list_elem *b, void *aux UNUSED); ///

static bool pri_more (const struct list_elem *a,const struct list_elem *b, void *aux UNUSED); ///
...
void 
thread_sleep (int64_t ticks) // == sleep until..
{
...
thread_unsleep (int64_t ticks){
  ...
  while(list_empty(&alarms) == false){
    struct alarm *a = list_entry (list_begin (&alarms), struct alarm, elem);
...
static bool 
exp_less (const struct list_elem *a,const struct list_elem *b, void *aux UNUSED)
{
